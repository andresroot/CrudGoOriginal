...
func ResolucionsoporteLista(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/resolucionsoporte/resolucionsoporteLista.html")
	log.Println("Error resolucionsoporte 0")
	db := dbConn()
	res := []Resolucionsoporte{}
	db.Select(&res, "SELECT * FROM resolucionsoporte ORDER BY cast(codigo as integer) ASC")
	varmap := map[string]interface{}{
		"res":     res,
		"hosting": ruta,
	}
	log.Println("Error resolucionsoporte888")
	tmp.Execute(w, varmap)
}

// RESOLUCION NUEVO
func ResolucionsoporteNuevo(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/resolucionsoporte/resolucionsoporteNuevo.html")
...
// RESOLUCION INSERTAR
func ResolucionsoporteInsertar(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	if r.Method == "POST" {
		...
		var q = "INSERT INTO resolucionsoporte(" +
			"codigo," +
			"numero," +
			"prefijo," +
			"tipo," +
	...
	http.Redirect(w, r, "/ResolucionsoporteLista", 301)
}

// RESOLUCION EXISTE
func ResolucionsoporteExiste(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	var total int
	row := db.QueryRow("SELECT COUNT(*) FROM resolucionsoporte  WHERE codigo=$1", Codigo)
	err := row.Scan(&total)
	if err != nil {
		log.Fatal(err)
	}
...
func ResolucionsoporteEditar(w http.ResponseWriter, r *http.Request) {
	Codigo := mux.Vars(r)["codigo"]
	log.Println("inicio resolucionsoporte editar" + Codigo)
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/resolucionsoporte/resolucionsoporteEditar.html")
	db := dbConn()
	t := Resolucionsoporte{}
	err := db.Get(&t, "SELECT * FROM resolucionsoporte where codigo=$1", Codigo)
	if err != nil {
		log.Fatalln(err)
	}
...
func ResolucionsoporteActualizar(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	err := r.ParseForm()
	if err != nil {
		panic(err.Error())
	...
	var q string
	q = "UPDATE resolucionsoporte set "
	q += "Numero=$2,"
	q += "Prefijo=$3,"
	q += "Tipo=$4,"
	q += "FechaInicial=$5,"
...
// RESOLUCION BORRAR
func ResolucionsoporteBorrar(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/resolucionsoporte/resolucionsoporteBorrar.html")
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	t := Resolucionsoporte{}
	err := db.Get(&t, "SELECT * FROM resolucionsoporte where codigo=$1", Codigo)
...
// RESOLUCION ELIMINAR
func ResolucionsoporteEliminar(w http.ResponseWriter, r *http.Request) {
	log.Println("Inicio Eliminar")
	db := dbConn()
	emp := mux.Vars(r)["codigo"]
	delForm, err := db.Prepare("DELETE from resolucionsoporte WHERE codigo=$1")
	if err != nil {
		panic(err.Error())
	}
	delForm.Exec(emp)
...
func ResolucionsoportePdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	t := Resolucionsoporte{}
	...
	var ciudadresolucionsoporte ciudad = TraerCiudad(t.Ciudad)
	...
	pdf.CellFormat(40, 4, "Ciudad", "", 0,
		"", false, 0, "")
	pdf.CellFormat(40, 4, ciudadresolucionsoporte.NombreCiudad, "", 0,
		"", false, 0, "")
	pdf.Ln(-1)
	pdf.SetX(30)
