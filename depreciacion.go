...
// DEPRECIACION  LISTA
func DepreciacionLista(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/depreciacion/depreciacion.html")
	db := dbConn()
	selDB, err := db.Query("SELECT * FROM depreciacion")
	if err != nil {
		panic(err.Error())
	}
	res := []depreciacion{}
	for selDB.Next() {
		...
		err = selDB.Scan(&Codigo, &Nombre, &Porcentaje, &Meses)
		if err != nil {
			panic(err.Error())
		}
		res = append(res, depreciacion{Codigo, Nombre, Porcentaje, Meses })
	}
...
func DepreciacionPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
