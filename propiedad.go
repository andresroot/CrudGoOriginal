...
func PropiedadLista(w http.ResponseWriter, r *http.Request) {
	...
	res := []propiedad{}
	res2 := []propiedad{}
	err :=db.Select(&res, "SELECT * FROM propiedad ORDER BY cast(codigo as integer) ASC")

	for _, x := range res {
		x.Acumulado=SumarPropiedad(x.Cuentagasto, x.Codigo)
...
func PropiedadNuevo(w http.ResponseWriter, r *http.Request) {
	...
	miTemplate, err := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/propiedad/propiedadNuevo.html", "vista/autocompleta/autocompletaTercero.html")
...
func PropiedadEditar(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/propiedad/propiedadEditar.html", "vista/autocompleta/autocompletaTercero.html")
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
...
func PropiedadPdf(w http.ResponseWriter, r *http.Request) {
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.SetX(20)
		pdf.CellFormat(184, 6, "Datos Propiedad Planta y Equipo", "0", 0,
			"C", true, 0, "")
		pdf.Ln(8)
	})
	pdf.AliasNbPages("")
...
func PropiedadTodosPdf(w http.ResponseWriter, r *http.Request) {
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.Ln(6)
		pdf.SetX(55)
		pdf.CellFormat(190, 10, "Propiedad, Planta y Equipo", "0", 0,
			"C", false, 0, "")
		pdf.Ln(10)
	})
...
func PropiedadExcel(w http.ResponseWriter, r *http.Request) {
	...
	f.SetCellValue("Sheet1", "A6","PROPIEDAD DE COSTOS")
